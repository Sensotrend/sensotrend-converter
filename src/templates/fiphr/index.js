...
export class FIPHRDataProcessor {
   ...
   async loadTemplate(objectType) {
      ...
      try {
         ...
         this.logger.error('FHIR conversion error: template for object type "' + objectType + '" not found');
         return;
      }
   ...
   async _convertRecordFromFHIR(sourceData, context) {
      ...
      try {
         ...
         this.logger.info('Hit an issue converting data on context: ' + context + ' ' + JSON.stringify(sourceData, null, 2) + ' ' + error);
      }
   }
